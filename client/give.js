...
Give = {
  getCleanValue: function( selector ) {
    const jqueryObjectVal = $( selector ).val();
  ...
  get_fee: function( amount ) {
    const r = (100 - 2.2) / 100;
    const i = (parseFloat( amount ) + 0.3) / r;
    const s = i - amount;
  ...
  process_give_form: function( quickForm, customer, user_id ) {
    ...
    if ( form.paymentInformation.donateWith === "Card" ) {
      ...
      const payment = { id: form.paymentInformation.donateWith };
  ...
  handleCalls: function( payment, form ) {
    ...
    Meteor.call( 'stripeDonation', form, function( error, result ) {
      if ( error ) {
        ...
        if ( result.error ) {
          const sendError = { code: result.error, message: result.message };
  ...
  updateSplitTotal() {
    ...
    $( "#total_amount" ).val(displaySum);
    Session.set( "total_amount", displaySum );
    Session.set( "giftAmount", displaySum);
    return sum;
  },
  updateTotal: function() {
    ...
    const splitAmounts = getCloneAmounts();
...
Template.registerHelper('org_street_address', function() {
  const streetAddressLine1 = OrgInfoCheck('address', 'line_1');
  const streetAddressLine2 = OrgInfoCheck('address', 'line_2');
...
Template.registerHelper('orgLogoURL', function() {
  const logoURL = OrgInfoCheck('logoURL', '');
  if (logoURL) {
    return logoURL;
  }
  return;
