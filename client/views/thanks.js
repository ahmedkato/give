...
Template.Thanks.helpers({
  ...
  displayReceipt: function() {
    const charge = Charges.findOne();
    const debitStatus = charge && charge.status;
    const refundStatus = charge && charge.refunded;
  ...
  failedTrans: function() {
    ...
    const referrer = Donations.findOne().URL;
    const errorMessage = Charges.findOne().failure_code ? Charges.findOne().failure_code + " " + Charges.findOne().failure_message : 'The error we got from the card \
