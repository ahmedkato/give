(function(namespace, $) {
  ...
  const p = AppOffcanvas.prototype;
	// =========================================================================
	// MEMBERS
	// =========================================================================
  ...
  p.openOffcanvas = function(id) {
    ...
    const leftOffcanvas = ($(id).closest('.offcanvas:first').length > 0);
    ...
    const translate = 'translate(' + width + 'px, 0)';
    $(id + '.active').css({
  ...
  p.toggleButtonState = function() {
    ...
    const id = $('.offcanvas-pane.active').attr('id');
  ...
  p.evalScrollbar = function() {
    ...
    const menu = $('.offcanvas-pane.active');
    if (menu.length === 0)			{
      return;
    }

		// Get scrollbar elements
    const menuScroller = $('.offcanvas-pane.active .offcanvas-body');
    const parent = menuScroller.parent();
    ...
    const height = $(window).height() - menu.find('.nano').position().top;
    const scroller = menuScroller.closest('.nano');
