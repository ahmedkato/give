(function($) {
  const App = function() {
    const o = this; // Create reference to this instance
  ...
  p.getKnobStyle = function(knob) {
    const holder = knob.closest('.knob');
    const options = {
  ...
  p._initAccordion = function() {
    ...
    $('.panel-group').on('show.bs.collapse', function(e) {
      const content = $(e.target);
      const card = content.parent();
      const group = card.closest('.panel-group');
  ...
  p._initInk = function() {
    ...
    $('.ink-reaction').on('click', function(e) {
      const bound = $(this).get(0).getBoundingClientRect();
      const x = e.clientX - bound.left;
      const y = e.clientY - bound.top;

      const color = o.getBackground($(this));
      const inverse = (o.getLuma(color) > 183) ? ' inverse' : '';

      const ink = $('<div class="ink' + inverse + '"></div>');
  ...
  p.getBackground = function(item) {
    ...
    const alpha = parseFloat(color.split(',')[3], 10);
  ...
  p.getLuma = function(color) {
    const rgba = color.substring(4, color.length - 1).split(',');
    const r = rgba[0];
    const g = rgba[1];
    const b = rgba[2];
  ...
  p.minBreakpoint = function(alias) {
    ...
    const breakpoint = $('#device-breakpoints div:visible').data('breakpoint');
  ...
  p._handleFunctionCalls = function(e) {
    ...
    for (let i = 0; i < this._callFunctions.length; i++) {
